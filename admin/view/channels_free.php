							...
							<?php foreach((array)$ayvpp_options['channels'] as $k => $v) { $d = empty($d) ? ' class="alternate"' : ''; ?>
								<tr id='field-<?php echo $k;?>'<?php echo $d;?>>
									<th scope='row' class="manage-column column-cb check-column">
										...
										<input type="hidden" name="name" value="<?php echo isset($v['name']) ? $v['name'] : ''; ?>" />
										<input type="hidden" name="channel" value="<?php echo isset($v['channel']) ? $v['channel'] : ''; ?>" />
										<input type="hidden" name="limit" value="<?php echo isset($v['limit']) ? $v['limit'] : ''; ?>" />
										<input type="hidden" name="type" value="<?php echo isset($v['type']) ? $v['type'] : ''; ?>" />
										<input type="hidden" name="publish" value=<?php echo isset($v['publish']) ? $v['publish'] : ''; ?> />
										<input type="hidden" name="import_description" value=<?php echo isset($v['import_description']) ? $v['import_description'] : ''; ?> />
										<input type="hidden" name="import_private" value=<?php echo isset($v['import_private']) ? $v['import_private'] : ''; ?> />
										<input type="hidden" name="author" value=<?php echo isset($v['author']) ? $v['author'] : ''; ?> />
					...
					<form id="WP_ayvpp_add_item_form" method="post" action="<?php bloginfo('wpurl'); ?>/wp-admin/admin.php?page=ayvpp-channels">
						...
						<div class="form-field">
							...
							<div class="categories"><div>
								<?php $r = get_categories([
									'hide_empty'	=>	false,
								]); ?>
								<?php foreach((array)$r as $k => $v) { ?>
								<label>
									<input type="checkbox" name="categories[]" class="chk" value="category|<?php echo $v->term_id; ?>" /> <?php echo $v->name; ?>
								</label>
								<?php } ?>
							</div></div>
						</div>
